...
export default function WorkoutLogger() {
  ...
  const activeEntryId = useMemo(() => {
    if (!activeWorkout || activeWorkout.entries.length === 0) {
      return null;
    }
    const exists = activeWorkout.entries.some((e) => e.id === selectedEntryId);
    if (exists && selectedEntryId !== null) {
  ...
  const handleStartCardioTemplate = async (template: WorkoutTemplate) => {
    await handleStartFromTemplate(template);
    setIsCardioSheetOpen(false);
  };

  /** 以計畫範本（或空白訓練）開始今天該練的那一格——改版前的既有行為 */
  const handleStartFromSlotTemplate = async () => {
    if (!activeProgram || !currentSlot) return;
    try {
      ...
      // 失敗（例如已有進行中訓練）也要收掉選單，不要讓它蓋在訓練畫面上
      setIsRecentSheetOpen(false);
    }
  };

  /** 沿用過去某一次的紀錄開訓（帶重量/次數），並帶回計畫資訊讓 cursor 與輪動照常計算 */
  const handleStartFromPastWorkout = async (source: Workout) => {
    if (!activeProgram || !currentSlot) return;
    try {
      await startWorkoutFromPastWorkout(source, {
        ...
        cycleNumber: activeProgram.cycleCount + 1,
      });
    } catch (err) {
      console.error(err);
      if (err instanceof Error && err.message === 'ACTIVE_WORKOUT_EXISTS') {
  ...
  /** 「開始今天訓練」：有過往紀錄就先讓使用者選要沿用哪一次，沒有就直接開訓 */
  const handleStartTodayWorkout = () => {
    if (recentSlotWorkouts.length === 0) {
      void handleStartFromSlotTemplate();
      return;
    }
    setIsRecentSheetOpen(true);
  };
  ...
  const summarizeWorkout = (workout: Workout) => {
    ...
    return {
      totalSets,
      preview: names.length > 3 ? `${preview}…` : preview,
    };
  };

  const formatWhen = (startedAt: number) => {
    const date = new Date(startedAt);
    const daysAgo = getCalendarDaysAgo(startedAt, now);
    const relative = daysAgo === 0 ? '今天' : daysAgo === 1 ? '昨天' : `${daysAgo}天前`;
  ...
  const handleSelectExercise = async (exercise: Exercise) => {
    if (rebindTargetEntryId) {
      ...
      const updatedWorkout = useActiveWorkoutStore.getState().activeWorkout;
      if (updatedWorkout && updatedWorkout.entries.length > 0) {
        const lastEntry = updatedWorkout.entries[updatedWorkout.entries.length - 1];
        if (lastEntry) {
        ...
        <div className="space-y-6 pb-24">
          ...
          <div className="space-y-4">
            {(() => {
                ...
                return (
                  <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-8 text-center shadow-sm">
                    <p className="text-slate-400 dark:text-slate-500 text-sm italic">
                      尚未加入任何動作，請點擊右上角「＋」加入第一個動作。
                    </p>
                  </div>
                );
              }
              
              const entry = activeWorkout.entries.find((e) => e.id === activeEntryId);
              ...
              return (
                <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden space-y-3 pb-3">
                  {/* 動作標頭 */}
                  ...
                  {/* UI ①: 替代動作列 */}
                  <div className="px-4 py-2 border-b border-slate-100/50 dark:border-slate-800/30 space-y-2">
                    {entry.candidateExerciseIds && entry.candidateExerciseIds.length > 1 && (
                      <div className="flex flex-wrap gap-1.5 items-center">
                        {entry.candidateExerciseIds.map((candId) => {
                          const candEx = allExercises.find((ex) => ex.id === candId);
                          ...
                          return (
                            ...
                            >
                              ...
                              {!isCurrent && (
                                <button
                                  type="button"
                                  onClick={() => removeAlternativeFromEntry(entry.id, candId)}
                                  className="pr-2 pl-0.5 py-1.5 hover:text-rose-500 font-bold transition text-[10px] cursor-pointer"
                                  title="移除此替代"
                                >
                                  ✕
                                </button>
                    ...
                    <div className="flex justify-start">
                      <button
                        ...
                        className="text-[11px] font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50/50 dark:bg-indigo-950/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 px-2.5 py-1.5 rounded-lg border border-dashed border-indigo-200 dark:border-indigo-800 flex items-center gap-1 transition cursor-pointer"
                      >
                            ...
                            <div
                              ...
                              className={`rounded-xl border p-2.5 space-y-2 transition duration-200 ${
                                setLog.completed
                                  ...
                                  : 'bg-slate-50/30 dark:bg-slate-800/20 border-slate-100 dark:border-slate-800'
                              }`}
                            >
                              {/* 第一列：組序 + 重量 + 次數 + 完成 */}
                    ...
                    <div className="flex gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                      ...
                      {entry.sets.length > 0 && (
                        <button
                          ...
                          className="px-3 py-2 bg-rose-50 dark:bg-rose-950/20 hover:bg-rose-100 dark:hover:bg-rose-900/40 text-rose-600 dark:text-rose-400 text-xs font-bold rounded-xl transition cursor-pointer"
                        >
                          － 刪除一組
                        </button>
                      )}
          ...
          <button
            ...
            className="w-full py-3.5 bg-indigo-50 dark:bg-indigo-950/20 hover:bg-indigo-100/80 dark:hover:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 font-bold rounded-2xl border border-dashed border-indigo-200 dark:border-indigo-800 flex items-center justify-center gap-2 transition cursor-pointer"
          >
            ...
            <div className="max-w-md mx-auto w-full px-4 py-4 space-y-2.5">
              {cardioTemplates.length === 0 ? (
                <p className="text-sm text-slate-400 dark:text-slate-500 text-center py-16 leading-relaxed px-6">
                  還沒有有氧範本。
                  <br />
                  做一次有氧訓練，完成時另存為範本，之後就能從這裡一鍵開始。
                </p>
              ) : (
                cardioTemplates.map((t) => {
                  const minutes = getTemplateTotalMinutes(t);
                  const totalSets = t.entries.reduce((sum, e) => sum + e.sets.length, 0);
                  return (
                    <button
                      key={t.id}
                      type="button"
                      onClick={() => handleStartCardioTemplate(t)}
                      className="w-full text-left bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl p-3 shadow-sm hover:shadow transition cursor-pointer space-y-1"
                    >
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-800 dark:text-slate-200 text-sm">{t.name}</span>
                        {t.location && (
                          <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold bg-indigo-50 dark:bg-indigo-950/40 px-1.5 py-0.5 rounded">
                            📍 {t.location}
                          </span>
                      ...
                      <p className="text-[10px] text-slate-400 font-medium">
                        {minutes > 0
                          ? `${t.entries.length} 個動作 • 共 ${minutes} 分鐘`
                          : `${t.entries.length} 個動作 • ${totalSets} 組`}
                      </p>
                    </button>
                  );
                })
