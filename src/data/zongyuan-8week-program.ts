...
export const ZONGYUAN_8WEEK_PLAN: ZongYuanDayPlan[] = [
  {
    ...
    exercises: [
      ...
      {
        planName: '滑輪下拉', exerciseName: '滑輪下拉（寬握）', muscleGroup: '背', equipment: '纜繩', isNewCustom: false,
  ...
  {
    label: '推 (Push)',
    weeklyTotalSets: ['12組', '12組', '12組', '6組', '12組', '12組', '12組', '3組'], // 扣除已移除的 3組「斜板推」
    exercises: [
      {
  ...
  },
  {
    ...
    exercises: [
      {
        planName: '三頭V把下壓', exerciseName: '纜繩下壓（平把）', muscleGroup: '手臂', equipment: '纜繩', isNewCustom: false,
...
// 這張表是教練**原始**課表的數字，不隨我們對執行課表的增刪重算。
// v1.11 移除了「斜板推」，所以胸大肌那列的數字仍含它 —— 標籤照實寫，不要改成別的動作名，
// 否則等於用一組數字去宣稱另一件事。
export const ZONGYUAN_COACH_CHECK_TABLE = {
  title: '教練原始容量覆核對照（各部位週總容量檢核）',
  rows: [
    { part: '胸大肌 (槓鈴臥推 + 斜板推・已移除)', values: [16, 17, 19, 8, 17, 18, 19, 10] },
